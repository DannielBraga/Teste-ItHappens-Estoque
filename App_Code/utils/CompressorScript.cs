...
public class ScriptCompressorHandler : IHttpHandler
{
	...
	private static string StripWhitespace(string body)
	{
        ...
        ////body = Regex.Replace(body, "/\\*.*?\\*/", String.Empty, RegexOptions.Compiled | RegexOptions.Singleline); // remove C styles comments
        //body = Regex.Replace(body, @"/[*][\w\d\s]+[*]/", string.Empty, RegexOptions.Multiline); // remove C styles comments
        //body = Regex.Replace(body, "^\\s*", String.Empty, RegexOptions.Compiled | RegexOptions.Multiline); // trim left
        //body = Regex.Replace(body, "\\s*[\\r\\n]", "\r\n", RegexOptions.Compiled | RegexOptions.ECMAScript); // trim right
        //body = Regex.Replace(body, "\\s*{\\s*", "{", RegexOptions.Compiled | RegexOptions.ECMAScript);// remove whitespace beside of left curly braced
        //body = Regex.Replace(body, "\\s*,\\s*", ",", RegexOptions.Compiled | RegexOptions.ECMAScript);// remove whitespace beside of coma
        //body = Regex.Replace(body, "\\s*;\\s*", ";", RegexOptions.Compiled | RegexOptions.ECMAScript);// remove whitespace beside of semicolon
        //body = Regex.Replace(body, "\\r\\n(?<=\\b(abstract|boolean|break|byte|case|catch|char|class|const|continue|default|delete|do|double|else|extends|false|final|finally|float|for|function|goto|if|implements|import|in|instanceof|int|interface|long|native|new|null|package|private|protected|public|return|short|static|super|switch|synchronized|this|throw|throws|transient|true|try|typeof|var|void|while|with)\\r\\n)", " ", RegexOptions.Compiled | RegexOptions.ECMAScript);// remove newline after keywords
...
public class ScriptCompressorModule : IHttpModule
{
	...
	private class WebResourceFilter : Stream
	{
		...
		private readonly Regex regex = new Regex("<script\\s*src=\"((?=[^\"]*(webresource.axd|scriptresource.axd|scriptservice.asmx))[^\"]*)\"\\s*type=\"text/javascript\"[^>]*>[^<]*(?:</script>)?", RegexOptions.IgnoreCase | RegexOptions.Compiled);
